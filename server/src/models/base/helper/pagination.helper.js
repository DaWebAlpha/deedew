...
/**
 * Defaults every paginated read to isDeleted: false — the single
 * place this rule is enforced, instead of every list service having
 * to remember it (a filter that's already been missed once). A
 * caller that genuinely wants deleted documents (an admin "trash"
...
const paginateCollection = async ({
    ...
    /**
     * Opt-in, not opt-out. .lean() bypasses toJSON entirely — plain
     * objects never go through a model's field-hiding transform
     * (Product.price, Payment.gatewayResponse). Defaulting to lean
     * would silently leak those the moment someone paginates that
     * model; a caller has to explicitly confirm a model has nothing
     * to hide before asking for the faster path.
     */
    if (lean === true) {
        query = query.lean();
    }
